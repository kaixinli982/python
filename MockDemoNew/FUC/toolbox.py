# -*- coding: utf-8 -*-
import sys,re
from mock_pro.models import Log
import random,datetime,urllib,urllib2,hashlib,time
from django.http import HttpResponse
from mock_pro.models import Person,Request,Log,Request_Data
# import pylab
import os
import json
import shutil
...
def uniteDict(src):
    ...
    key_list = []
    for key in src:
        if re.findall("f_id",key):
            key_list.append(src[key])
    all_dict ={}
    for key in key_list:
        temp_dict={}
        for temp_key in src:
            if re.findall(key,temp_key):
                temp_dict[temp_key.split("_",2)[2]]=src[temp_key]
        all_dict[key]=temp_dict
    print '&&&&&&&&&',all_dict
    return  all_dict
...
def requestDict(temp,type_="normal"):
    ...
    if type_=="normal":
        for i in temp:
            ...
            if data_temp:
                ...
                data[str(i.key)]=(''if i.value=="null"or "None"==str(i.value) else str(i.value))
    elif type_=="dict":
        for i in temp:
    ...
    # data = str(json.dumps(data, encoding='utf-8', ensure_ascii=False))
    print "requestDict调用结束data:",type(data),data,"header:",header
    return data

#时间戳装饰器
def reqLog(arg):
    start = time.time()
    def _deco(func, start=start):
        def __deco(*args, **kwargs):
            apicontent = func(*args, **kwargs)
            logInsert(args[0].GET['url'], apicontent[1], arg,
                              Request.objects.get(id=kwargs["mock_pro_id"]).name, apicontent[0],
                              Request.objects.get(id=kwargs["mock_pro_id"]), str(time.time() - start), str(len(apicontent[0])))
            return HttpResponse(apicontent[0])
        return __deco
    return _deco
def xxdData(data_src):
    data_src = str(data_src)
    data_src = data_src.replace("u'", "\"")
    data_src = data_src.replace("'", "\"")
    data_src=str({"data": str(data_src)}).replace("'",'''"''')
    data_src=data_src.replace("\"{","{")
    data_src =data_src.replace("}\"}","}}")
    ...
    print "data_src!!!!!!!",data_src
    return data_src
def xxdFile(file_src):
    file_src = str(file_src)
    file_src = file_src.replace("u'", "\"")
...
def xxd_reqLog(arg):
    ...
    def _deco(func, start=start):
        def __deco(*args, **kwargs):
            ...
            print args
            print apicontent
            logInsert(args[0].GET['url'], apicontent[1], args[2],
                              Request.objects.get(id=args[1]).name, apicontent[0],
                              Request.objects.get(id=args[1]), str(time.time() - start), str(len(apicontent[0])))
            print "装饰器操作完成退出"
            return apicontent[0],apicontent[1]
        return __deco
    return _deco
...
def reqGETDict(temp,type_="normal"):
    ...
    if type_ == "normal":
        for i in temp:
            ...
            elif i[:5].lower() == "file_":
                print i,"file"
                file_src[i[5:]] = temp[i]
            elif i != 'url' and i !='sign' and i!='text' and i!='backup':
                data_src[i]=temp[i]
        ...
        for i in temp:
            if i[:7].lower()=="header_":
                ...
                data_src[i]=temp[i]
    print data_src,header_src,file_src
    return data_src,header_src,file_src
#入参嵌套list，dict处理
def data_nest(data):
    data=data['data']
    for k,v in a.items():
...
#根据来源接口数据透传
def data_trans(data, backup, text):
    ...
    backup = eval(backup)
    for key in backup:
        value_list = backup[key].split("_")
        if 'req'==value_list[0]:
            a=value_list[1]
            x=dict_get(text,a,"none")
            print x
            data[key] = x
            print "根据来源接口数据透传成功"
    return data
...
def str_dic(data):
    ...
    data=eval(data)
    return data
#判断url是否带可变参数然后重新组装url
def get_url_pro(prc_url,temp):
    new_url=prc_url
    prc_url=str(prc_url)
    prc_url_tmp=prc_url.split('/')
    u1={}
    for item in prc_url_tmp:
        start = item.find('{')
        end = item.find('}')
        if start!=-1&end!=-1:
            parmar=item[start:end + 1]
            parmar1=parmar[1:-1]
            u1[parmar1]=parmar
    for k,v in u1.items():
        newpat=v
        pat=temp[k]
        new_url=re.sub(newpat,pat,prc_url)
    return new_url
